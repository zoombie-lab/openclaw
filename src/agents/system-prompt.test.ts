...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes the canonical file workflow when save_file is available", () => {
    ...
    expect(prompt).toContain("canonical file tool");
    expect(prompt).toContain("Before `image_generate`");
    expect(prompt).toContain("media`, `path`, or `filePath`");
  });
  ...
  it("includes the current date/time block when provided", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/clawd",
      userTimezone: "America/Chicago",
      userTime: "Monday, January 5th, 2026 — 3:26 PM",
