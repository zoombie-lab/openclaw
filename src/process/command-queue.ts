        ...
        void (async () => {
          ...
          try {
            ...
            if (completedCurrentGeneration) {
              diag.debug(
                `lane task done: lane=${lane} durationMs=${Date.now() - startTime} active=${
...
export function recoverCommandLane(lane: string = CommandLane.Main) {
  const cleaned = lane.trim() || CommandLane.Main;
  const state = queueState.lanes.get(cleaned);
  if (!state) {
    return { activeRecovered: 0, queued: 0 };
  }
  const activeRecovered = state.activeTaskIds.size;
  state.generation += 1;
  state.activeTaskIds.clear();
  state.draining = false;
  if (activeRecovered > 0) {
    diag.warn(
      `lane recovered: lane=${cleaned} activeRecovered=${activeRecovered} queued=${state.queue.length}`,
    );
  }
  if (state.queue.length > 0) {
    drainLane(cleaned);
  }
  return { activeRecovered, queued: state.queue.length };
}
...
export function waitForActiveTasks(timeoutMs: number): Promise<{ drained: boolean }> {
  ...
  return new Promise((resolve) => {
    const check = () => {
      let pendingTasks = 0;
      for (const state of queueState.lanes.values()) {
        pendingTasks += state.activeTaskIds.size + state.queue.length;
      }
